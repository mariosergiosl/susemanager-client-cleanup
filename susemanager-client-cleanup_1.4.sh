...
print_help() {
 ...
 DESCRIPTION: This script removes a SUSE Manager client from the system. It can be used on cloned systems, 
       systems with registration issues, or systems with configuration problems.

 OPTIONS:

 IMPORTANT: At least one of the options -A, -nB, -nC, -nD, -oC, or -nT must be specified.
...
# Check if any of the mandatory options were used
if [[ "$use_option_A" == "false" && "$use_option_nB" == "false" && "$use_option_nC" == "false" && "$use_option_nD" == "false" && "$use_option_oC" == "false" && "$use_option_nT" == "false" ]]; then
  echo "Error: At least one of the options -A, -nB, -nC, -nD, -oC, or -nT must be specified."
  print_help
  exit 1
fi
...
if [[ "$use_option_nB" == "false" && "$use_option_nT" == "false" ]]; then
 echo "#######################################################################################"
